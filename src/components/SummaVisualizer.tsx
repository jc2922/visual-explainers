...
export const SummaVisualizer: React.FC = () => {
    ...
    switch (step) {
      case AlgorithmStep.DISTRIBUTION:
        return {
          ...
          desc: "the input matrices are divided into regional blocks. each node takes ownership of a local segment of A and B."
        };
      case AlgorithmStep.BROADCAST_A:
        return {
          title: "2. row broadcast",
          desc: `iteration k=${currentK}. the source node in each row propagates its A-block across the mesh topology.`
        };
      case AlgorithmStep.BROADCAST_B:
        return {
          title: "3. column broadcast",
          desc: `simultaneously, the source node in each column propagates its B-block vertically through the network.`
        };
      case AlgorithmStep.COMPUTE:
        return {
          title: "4. parallel compute",
          desc: "each node performs a local multiply-accumulate operation on its newly inherited data blocks."
        };
      case AlgorithmStep.FINISHED:
        return {
          title: "simulation complete",
          desc: "distributed matrix product C has been computed across the parallel grid."
        };
      default:
        return { title: "", desc: "" };
    }
  ...
  return (
    <div className="h-screen w-full bg-zinc-950 text-zinc-300 font-sans flex flex-col overflow-hidden selection:bg-emerald-500/30">
      ...
      {/* Header Section */}
      <header className="h-16 border-b border-zinc-900 bg-zinc-950 flex items-center justify-between px-6 shrink-0 z-20 shadow-xl">
        <div className="flex items-center gap-4">
          <button 
            onClick={() => navigate('/')}
            className="p-2 hover:bg-zinc-900 rounded-lg text-zinc-600 hover:text-white transition-colors mr-2"
          >
            <ChevronLeft size={20} />
          </button>
          <div className="flex items-center gap-3">
            <Cpu size={24} className="text-emerald-500" />
            <span className="text-sm font-bold tracking-[0.2em] uppercase text-white">Visual.Explainer</span>
          </div>
          <div className="w-px h-6 bg-zinc-900 mx-2" />
          <div>
            <h1 className="text-xs font-bold text-zinc-400 leading-tight uppercase tracking-widest">SUMMA Matrix Multiplier</h1>
          </div>
        </div>

        <div className="flex items-center gap-4">
          <div className="flex flex-col items-end mr-2">
            <span className="text-[8px] text-zinc-600 uppercase font-bold tracking-widest leading-none mb-1">Matrix Size</span>
            <select 
              value={matrixSize} 
              onChange={(e) => setMatrixSize(Number(e.target.value))}
              disabled={isPlaying}
              className="bg-zinc-950 border border-zinc-900 text-zinc-500 text-[10px] px-2 py-1 rounded cursor-pointer outline-none focus:border-emerald-500 transition-colors"
            >
          ...
          <div className="flex flex-col items-end mr-2">
            <span className="text-[8px] text-zinc-600 uppercase font-bold tracking-widest leading-none mb-1">Mesh Grid</span>
            <select 
              value={processorSize} 
          ...
          <div className="flex flex-col items-end mr-4">
            <span className="text-[9px] text-zinc-600 uppercase font-bold tracking-widest leading-none mb-1">Status</span>
          ...
          <div className="flex items-center gap-2 bg-zinc-950 border border-zinc-900 p-1 rounded-lg">
            <button 
              onClick={reset}
              className="p-2 hover:bg-zinc-900 rounded text-zinc-700 hover:text-white transition-colors"
            >
              <RotateCcw size={16} />
            </button>
            <button 
            ...
            <button 
              ...
              className="p-2 hover:bg-zinc-900 rounded text-zinc-600 hover:text-white transition-colors disabled:opacity-20"
            >
      ...
      <div className="flex flex-1 min-h-0 bg-zinc-900 overflow-hidden">
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={idx} className="flex justify-between items-center text-[10px]">
                  <span className="text-zinc-500 uppercase font-bold">{stat.label}</span>
                  <span className="text-white font-mono bg-zinc-900 px-1.5 py-0.5 rounded border border-zinc-800">{stat.val}</span>
                </div>
              ))}
            </div>
          </section>
        ...
        <main className="flex-1 overflow-y-auto relative flex flex-col p-6 lg:p-10 scrollbar-hide bg-zinc-950">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-12 items-start mt-4">
            ...
            <div className="xl:col-span-4 space-y-10">
              <section className="space-y-3">
                ...
                <div className="grid gap-1 p-2 bg-zinc-950 rounded-xl border border-zinc-800 shadow-inner" style={{ gridTemplateColumns: `repeat(${GRID_SIZE}, minmax(0, 1fr))` }}>
                  {matrixA.map((row, i) => 
                    row.map((val, j) => {
              ...
              <section className="space-y-3">
                ...
                <div className="grid gap-1 p-2 bg-zinc-950 rounded-xl border border-zinc-800" style={{ gridTemplateColumns: `repeat(${GRID_SIZE}, minmax(0, 1fr))` }}>
                  {matrixB.map((row, i) => 
                    row.map((val, j) => {
                      const peRow = Math.floor(i / BLOCK_SIZE);
                      const peCol = Math.floor(j / BLOCK_SIZE);
            ...
            <div className="xl:col-span-8 space-y-4">
              ...
              <div className="grid gap-4 p-6 bg-zinc-950 rounded-3xl border border-zinc-800 relative shadow-2xl min-h-[500px]" style={{ gridTemplateColumns: `repeat(${P_GRID}, minmax(0, 1fr))` }}>
                {processors.map((p) => {
        ...
        <aside className="hidden xl:flex w-72 border-l border-zinc-800 bg-zinc-950 p-5 flex-col gap-6 shrink-0 overflow-y-auto">
          ...
          <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-4 space-y-4">
            ...
            <div className="font-mono text-[11px] space-y-3 leading-relaxed font-bold">
              <div className="flex items-center gap-3">
          ...
          <section>
            ...
            <div className="h-28 bg-zinc-900 border border-zinc-800 rounded-xl p-3 flex flex-col justify-between overflow-hidden relative group">
              <div className="flex justify-between items-center text-[10px]">
                <span className="text-zinc-500 tracking-widest uppercase text-[8px]">M-Cast Ring</span>
          ...
          <footer className="mt-auto pt-6 border-t border-zinc-800">
            <div className="p-3 bg-emerald-500/5 border border-emerald-500/10 rounded-lg">
              <div className="text-[9px] text-emerald-500 font-bold uppercase mb-1.5 flex items-center gap-1.5 tracking-widest">
